...
getMeasuresIn <- function(category) {
	...
	endpoint = "http://logd.tw.rpi.edu/sparql.php";
	return (as.character(SPARQL(url=endpoint,query=measuresQuery)$results$measure));
...
#auto_derived_set <- function(){
	...
	endpoint = "http://logd.tw.rpi.edu/sparql.php";
	states = SPARQL(url=endpoint,query=measuresQuery)$results$o;
	...
	#getMeasuresIn("http://health.tw.rpi.edu/source/impacteen-org/dataset/tobacco-control-policy-and-prevalence/category/Youth_Risk_Behavior_Surveillance_System");
	print (behaviors);
